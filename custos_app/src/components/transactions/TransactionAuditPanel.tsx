...
export function TransactionAuditPanel({ tx }: { tx: Transaction }) {
  ...
  return (
    <div className="space-y-6">
        ...
        >
          ...
          <div>
            ...
            {tx.status === "pending_review" && (
              <Link href="/review-queue" className="mt-2 inline-block text-sm underline underline-offset-4">
                Open review queue
              </Link>
            )}
          </div>
      ...
      {payoutLbl && (
        <p className="text-body-sm text-muted-foreground rounded-md border border-border bg-muted/30 px-3 py-2">
          {payoutLbl}
        </p>
      )}
      ...
      {(tx.riskScore != null ||
        ...
        <Card className="border-primary/20 bg-primary/[0.03]">
          ...
          <CardContent className="space-y-3 text-body-sm">
            ...
            {tx.riskFlags && tx.riskFlags.length > 0 && (
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Risk flags</p>
                ...
                <ul className="mt-2 space-y-2">
                  {(tx.citedRules as CitedRule[]).map((r) => (
                    <li key={r.id} className="rounded-md border border-border p-2 text-body-sm">
                      ...
                      {r.source && (
                        <p className="mt-1 text-caption text-muted-foreground">Source: {r.source}</p>
                      )}
                      {r.excerpt && (
                        <p className="mt-1 border-l-2 border-primary/30 pl-2 text-caption italic">{r.excerpt}</p>
                      )}
                    </li>
                  ))}
                </ul>
        ...
        <CardContent className="space-y-3 text-body-sm">
          ...
          {tx.sourceKind && (
            <p>
              ...
              <span className="capitalize">{tx.sourceKind.replace(/_/g, " ")}</span>
            </p>
          )}
      ...
      {tx.policyResult && (
        <Card>
          ...
          <CardContent>
            ...
            {tx.walletPolicy?.limits && (
              <div className="mb-3 rounded-md border border-border bg-muted/30 p-3 text-body-sm">
                <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">
                  Wallet limits
                </p>
                <div className="mt-2 space-y-1">
                  <p>
                    ...
                    <span className="capitalize">{tx.walletPolicy.approvalMode}</span>
                  </p>
                  <p>
                    <span className="text-muted-foreground">Daily limit:</span>{" "}
                    {tx.walletPolicy.limits.daily != null ? (
                      <span className="tabular-nums">
                        {formatCurrency(tx.walletPolicy.limits.daily, tx.currency)}
                      </span>
                    ) : (
                  ...
                  <p>
                    ...
                    {tx.walletPolicy.limits.perTransaction != null ? (
                      <span className="tabular-nums">
                        {formatCurrency(tx.walletPolicy.limits.perTransaction, tx.currency)}
                      </span>
                    ) : (
                      <span className="italic text-muted-foreground">Not set</span>
                    )}
      ...
      {tx.evidence && tx.evidence.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Evidence supplied with the request. Invoice images load when a <code className="text-xs">fileId</code>{" "}
              is present.
            </CardDescription>
          </CardHeader>
          <CardContent>
  ...
  return (
    <Card className="border-primary/35 bg-primary/[0.06] shadow-sm">
      <CardHeader className="pb-2">
        ...
        <CardDescription>
          Approve to mark this spend allowed. Decline to block it. Optional note is stored on the audit timeline.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <div className="flex flex-wrap gap-2">
          <Button type="button" disabled={mutation.isPending} onClick={() => mutation.mutate("approve")}>
