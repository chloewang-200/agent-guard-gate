...
export function EvidenceFilePreview({ fileId, filename }: { fileId: string; filename?: string }) {
  ...
  if (state === "not_image") {
    return (
      <p className="mt-2 text-caption text-muted-foreground">
        No image preview for this file type; the file is still attached as evidence.
      </p>
    );
  }
  ...
  return (
    <div className="mt-2 overflow-hidden rounded-md border border-border bg-muted/20">
      ...
      />
      {mime && (
        <p className="border-t border-border px-2 py-1 font-mono text-[10px] text-muted-foreground">{mime}</p>
      )}
    </div>
  );
}
