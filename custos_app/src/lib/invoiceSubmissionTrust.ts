...
/**
 * Default citations + decision trace for invoice submissions so the audit panel is populated
 * without a separate LLM round-trip (same pattern as empire custos_fe).
 */
export function buildInvoiceTrustFields(opts: {
  ...
  /** Empty string when operator did not pick an approved payee override */
  ...
  const citedRules: NonNullable<RequestTransactionBody["citedRules"]> = [
        ...
        },
    {
      ...
      excerpt: [
        ...
        conf != null ? `Pipeline extraction confidence ${conf}%.` : null,
      ]
        .filter(Boolean)
        .join(" "),
    },
  ...
  const agentDecision: NonNullable<RequestTransactionBody["agentDecision"]> = {
    ...
    reasoning: [
      ...
      extraction.railType
        ? `Suggested payout rail from extraction: ${extraction.railType.replace(/_/g, " ")}.`
        : null,
    ]
      .filter(Boolean)
      .join(" "),
